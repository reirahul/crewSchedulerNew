...
public class ImportHelper {
	 ...
	 * 
	 */
	public static void checkSugarVersion(String url) {
		String servVersion = "";
		try {
			SOAPClient sc = new SOAPClient(SoapHelper.generateUrl(url));
			servVersion = SOAPClient.getServerVersion();

			String ver = String.valueOf(servVersion.charAt(0));
			UserPreferences.usingV2Soap = (!servVersion.equalsIgnoreCase("") && Integer.parseInt(ver) >= 6 );
	...
	public static String importDatabase(Context context) {
		...
		if(importFields()){
			...
			if (UserPreferences.usingV2Soap) {
				return ImportHelperV2.performSync(context, true);
			} else {
